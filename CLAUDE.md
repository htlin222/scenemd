...
- Figures default to `legend` layout. Text that must share a figure's scene is wrapped explicitly in `<!-- present: group -->` … `<!-- present: end-group -->` (blocks get a shared `groupId`; the planner never places a scene boundary inside a group and flags an over-capacity group scene with a warning instead of shrinking the figure). There is no implicit neighbor gluing.
